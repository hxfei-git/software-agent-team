...
测试结论必须以 `requirements.md` 中的需求 ID（`Rxx`）和验收标准 ID（`Axx`）为依据。实现行为、接口、输出、数据格式或交付物与用户描述不一致时，必须判定 FAIL。
...
## 验收标准覆盖
| 需求 ID | 验收标准 ID | 验收标准 | 结果 | 证据 |
|---------|-------------|----------|------|------|
| R01 | A01 | {标准} | PASS | {文件/命令/行为} |

## 需求符合度
| 需求 ID | 用户描述要点 | 实际表现 | 结果 |
|---------|--------------|----------|------|
| R01 | {要点} | {证据摘要} | PASS |
```
...
## 问题列表
| # | 严重度 | 需求/验收 ID | 位置 | 问题 | 建议 |
|---|--------|--------------|------|------|------|
| 1 | 严重/中等/轻微 | R01/A01 | {文件或命令} | {描述} | {建议} |
