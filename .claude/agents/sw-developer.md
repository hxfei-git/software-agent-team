...
`requirements.md` 是唯一需求依据。你必须按当前任务对应的需求 ID（`Rxx`）和验收标准 ID（`Axx`）实现，不得添加未列入需求的功能、行为、接口、输出格式或交付物。
...
2. `requirements.md` 中相关原始需求摘录、团队理解版需求、需求 ID 和验收标准 ID
...
- 优先复用已有模块，不随意引入新依赖。
- 不做需求外功能。
- 不改变用户指定的行为语义、接口契约、输出格式、数据格式或交付边界。
- 如果当前需求冲突、不可实现、不可测试或缺少关键边界，停止开发并向主 Agent 返回阻塞原因。
- 涉及公共接口时更新相关调用方。
- 涉及用户可见行为时补充或更新测试。
- 对高风险改动给出简短说明。
...
1. 读取失败报告。
2. 回到 `requirements.md` 和 `dev-plan.md` 核对失败项对应的需求 ID / 验收标准 ID。
3. 定位相关代码。
4. 必要时调用 Codex 分析失败原因和修复方案。
5. 一次性修复所有报告中的问题，不顺带添加需求外功能。
6. 运行必要自测。
7. 将可复用经验追加到 `lessons-learned.md`。
